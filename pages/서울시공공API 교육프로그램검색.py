...
def get_seoul_education_services(api_key, selected_date, start_index=1, end_index=100):
    # API 요청 URL 생성
...
# Streamlit UI
def main():
    st.title("서울시 전체 교육 서비스 찾기")
    api_key = "767063424f7361793131334167597278"
    
    # 날짜 입력받기
    selected_date = st.date_input("Select Date (YYYY-MM-DD)", value=datetime(2024, 1, 1))
    
    if st.button("Search Services"):
        get_seoul_education_services(api_key, selected_date.strftime("%Y-%m-%d"))
